...
public class Combione {
    ...
    // //not pick-fun(4,1,{2,2,2}) //as index value is increased so we wiil not call
    // nothing
    ...
    public static void find(int ind, int[] arr, int target, List<List<Integer>> li, List<Integer> ds) {
        // so question talk about that we have to retrun list whose sum is equal to
        // target sum and base condition for that is is ind reaches arr.length then
        // recursion call will be end and if target result to zero then add that li tol
        // list of lsit
        if (ind == arr.length) {
            if (target == 0) {
        ...
        // given index then add the element in list and do not increase the ind as we
        // can take elemt multiple times so call th function
        // after when rexrsion is completed then removing the element from it for
        // backtracking
        if (arr[ind] <= target) {
            // picking an element
            ds.add(arr[ind]);
        ...
        // here come when we are not picking the element from then index will be
        // increase
